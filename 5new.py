...
def dE_dS(y, z, s, x, i, beta):
    ...
    for p in range(4):
        ...
        suma_1 = suma_1 + ((y[p] - z[p]) * df_dx(suma_wew, beta) * x[p][i])
    return suma_1

def dE_Dwij(y, z, s, x, w, u, i, j, beta):
    suma = 0.0
    ...
    for p in range(4):
        ...
        suma = suma + (y[p] - z[p]) * suma_wew1 * s[i] * suma_wew2 * u[p][j]
...
def main_func(u, z, beta, c, epsilon, w_old, s_old):
    xp = x_p(u, w_old, beta)
    yp = y_p(xp, s_old, beta)
    s_new = []
    w_new = []
    for i in range(3):
        s_new.append(s_old[i] - c * dE_dS(yp, z, s_old, xp, i, beta))

    for i in range(2):
        w = []
        for j in range(3):
            w.append(w_old[i][j] - c * dE_Dwij(yp, z, s_old, xp, w_old, u, i, j, beta))
        w_new.append(w)

    return w_new, s_new

def xor_bp(u, z, beta, c, epsilon, w_old, s_old):
    w_new, s_new = main_func(u, z, beta, c, epsilon, w_old, s_old)
    while True:
        max_w = 0
        w_roznicy = []
        for i in range(2):
